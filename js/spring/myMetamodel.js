...
function parseAndRefreshGraph(definitionsText, updateGraphFn, updateErrorsFn) {
    ...
    if (updateErrorsFn instanceof Function) {
        updateErrorsFn(null);
    }

    if (definitionsText.trim().length === 0) {
        if (updateGraphFn instanceof Function) {
            updateGraphFn({
                'format': 'xd',
                'streamdefs': [],
                'nodes': [],
    ...
    var parsedStreamData = parse(definitionsText);
...
function buildGraphFromJson(jsonFormatData, metamodel) {
    ...
    for (var n = 0; n < inputnodesCount; n++) {
        ...
        if (!group) {
            group = matchGroup(metamodel, name, incoming[n], outgoing[n]);
        }
        /* var newNode = flo.createNode(getMetadata(metamodel, name, group), inputnodes[n].properties); */
        var newNode = createStreamModel(getMetadata(metamodel, name, group), inputnodes[n].properties);
        ...
        if (streamname) {
            ...
            newNode.attr('.stream-label/text',streamname);
        }
    ...
    for (var l = 0; l < inputlinksCount; l++) {
        ...
        var from = {};
        from.element = graph.getCell(nodesIndex[link.from]);
        from.ports = from.element.getPorts();
        for (var j = 0; j < from.ports.length; j++) {
            if (from.ports[j] && from.ports[j].port === linkType) {
                from.portId = from.ports[j].id;
            }
        }

        var to = {};
        to.element = graph.getCell(nodesIndex[link.to]);
        to.ports = to.element.getPorts();
        for (var j = 0; j < to.ports.length; j++) {
            if (to.ports[j] && to.ports[j].port === 'input') {
    ...
    graph.resetCells(nodes.concat(links));
    graphLayout();  
}
                ...
                if (parsedNode.sourceChannelName) {
                    ...
                    if (channelText.startsWith('tap:')) { // TAP SOURCE
                        ...
                        graphNode = {
                            ...
                            'name': 'destination',
                            'properties': {
                                'name': parsedNode.sourceChannelName
                            }
                        };
                        linkFrom = graphNode.id;
                    }
...
function matchGroup(metamodel, type, incoming, outgoing) {
