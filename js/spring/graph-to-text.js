 ...
 */
    ...
    function isTapLink(link) {
		...
		return graph.getCell(link.get('source')).getPort(link.get('source').port).port === 'tap';
    }
	...
	function getOutgoingStreamLinks(node) {
		...
		if (node) {
			g.getConnectedLinks(node, {outbound: true}).forEach(function(link) {
				...
				if (graph.getCell(source.id).getPort(source.port).port === 'output') {
					links.push(link);
				}
			});
		}
	...
	function getOutgoingTapLinks(node) {
		...
		if (node) {
			g.getConnectedLinks(node, {outbound: true}).forEach(function(link) {
				...
				if (graph.getCell(source.id).getPort(source.port).port === 'tap') {
					links.push(link);
				}
			});
		}
	...
	function createTextForNode(element, first) {
		...
		var props = element.attributes.properties;
		if (!element) {
			return;
		}
